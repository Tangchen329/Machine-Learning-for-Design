...
到目前为止，我们讨论的界面种类非常像传统意义上的地图。像任何其他地图一样，我们可以添加文字标签，例如路标。 这样我们就可以用命令来导航设计空间：“把我带到枫叶”，一旦这样，我们就可以说：“让我靠近一个橡树叶。”这种方式本身就是强大的。 但是，事实证明，我们可以将这个想法更进一步。

2013年，[Tomas Mikolov](https://scholar.google.com/citations?user=oBu8kMMAAAAJ)等人发表了一系列文章，描述了生成表示词汇概念关系的低维图的技术。很像我们一直在讨论的地图，在现实世界的使用中紧密相关的词也将在嵌入的地图中彼此靠近。更令人难以置信的是，Mikolov和他的合作者发现有可能将概念上有意义的代数变换应用于字矢量。换句话说，他们发现你可以在现实世界的概念上执行代数。

例如，他们显示了词向量表达式的结果：“马德里 - 西班牙+法国”，比任何其他词向量更接近“巴黎”。然后：“王 - 男人+女人”导致一个非常接近“女王”的矢量。这个迷人的机制为设计工具提供了一种全新的思考方式。 它允许我们在视觉或语言上操作视觉概念，而不使用辅助抽象和控制系统。

例如，如果我们正在寻找像毕加索的审美，而不是从他（分析）立体派时期的高度，我们可以说：

图
我们可以对听觉信息或任何其他媒介做同样的事情。
在过去几年中，类似的技巧，包括“风格转移”和“神经涂鸦”，这些机制进一步扩大。

这些技术已经在照片共享应用程序中实现 - 不是作为更广泛的设计工具中的功能，而是作为一种新颖的图像过滤器，有些像Instagram或Photoshop过滤器。20世纪90年代，Photoshop“过滤器泡沫”证明，这种功能上的新颖性表现很快就变成了拙劣的东西，对于重新概念化或扩展设计过程的意义很小。

图

但是，作为一个更大更多设计框架的单个组件，这些技术提供了一种强大的机制来操作媒体。这种机制不离开本机词汇，也不将其映射到抽象上。它们允许我们通过直接操纵这些想法所在的概念空间来探索和组合想法。但是，作为技术的变革，我认为还有一些缺失的东西。每个设计师都知道设计中最难的事情不是做个人决定所涉及到的。 困难的部分是将许多组成部分的决定相互调和，以产生一个凝聚力的整体。

作为设计师，我们必须在许多组件决策之间来回移动，同时牢记整体。 有时这些组件决策相互冲突。

图

像魔方一样，我们不能简单地解决多维数据集的一面，然后移到下一个。 这将导致我们撤销我们早期的一些工作。 我们必须同时解决各方面。这可能是一个非常复杂的过程，学习导航它是成为设计师意义的核心。虽然我们讨论的机器学习技术可以帮助简化这些组件决策，但是并没有完全解决设计中最困难的一个方面。为了帮助设计者建立这种专业知识，我们再来探讨两个概念：

#### 过程组织

相比较复杂的，涉及多方面的语句，机器学习系统更容易理解传达单个命令或信息点的简单表达式。 然而，设计事物中最困难的过程之一就是思考如何将一个复杂的，动态的系统分解成离散的部分。设计工具可以做的最有用的事情之一是帮助设计者完成这一过程。工具可以帮助设计者通过创建界面和工作流来提供简洁的语句，从而引导用户通过一系列简单的练习或决策点，每个单独的练习或决策点都能处理更大更复杂任务的单一方面。这个方法的一个很好的例子是20Q，一个电子版的游戏二十个问题。

图

像原来的旅行游戏一样，20Q要求用户想到一个物体或者名人，然后提出一系列的选择题，以便发现用户的想法。这个过程中提出的第一个问题总是：“它被归类为动物，蔬菜，矿物还是概念？随后的问题试图揭示从用户已经提供的信息中扩展的进一步区分。例如，如果第一个问题的答案是“动物”，则下一个问题可能是“这是哺乳动物吗？如果相反，第一个答案是“蔬菜”，下一个问题可能是“它通常是绿色的”吗？

以下这些后续问题可以回答：是，否，有时或不相关。20Q在二十个问题之后80％的时间猜测正确的人，地点或事物，二十五个问题后98％的时间猜测。该系统使用一种称为学习决策树的机器学习算法来确定可能在最小步数中导致正确答案的问题序列。

图

使用先前用户与系统交互生成的数据，该算法学习了每个问题的相对值，以尽可能多地删除不正确的选项，以便能够向用户提出最重要的问题。例如，如果机器已经知道用户脑中想的是一个名人，下一个问题可能是这个人是否活着，而不是这个人是否写了一本书，因为只有一小部分 历史人物今天还活着，但许多名人都写过书。虽然这些问题都没有单独地封装用户脑海的全部内容，但相对较少数量的精心挑选的问题可以以惊人的速度发现正确答案。

除了帮助系统对用户表达的理解之外，这一过程可以直接增强用户更清晰，有目的地传达想法的能力。

图

其核心在于，这一过程可以被看作是通过大量相互关联的决策发现最佳路径的机制。每个问题和答案交互作为通过概念空间的翻译向量，使用户更接近他或她的预期输出，同时也探测用户思考和阐述该想法的每个方面。该机制也可以扩展到设计界面，允许用户通过回答一系列相关问题来以所需形式进行磨合。 借鉴自然的互动模式，这些问题可以口头回答...

或者用手势回答，避免用户需要学习复杂的菜单系统来访问该工具的功能：

图


基于机器学习的最新进展，机器越来越可能回答用户关于设计属性的复杂的上下文问题：

图

例如，用户可以提出有助于评估设计适用于某些预期用途的事实问题：

图

这种对话将模仿人类对话的形式，但将受益于机器对设计属性的无所不知的知识。这也可以与机器的模型模拟现实世界的约束条件，如材料，物理或化学方法能力的能力相关：

图

通过将此功能嵌入实时交互中，例如，建筑师可以通过快速消除不太可能产生丰硕成果的新兴想法来节省大量时间。除了“现实世界的约束”之外，用户的含义在给定的交互中可能并不总是很清楚 - 要么是因为机器的知识限制，要么是因为用户声明缺乏明确性：

图

机器可以提供澄清的问题和替代方案，而不是“最好的猜测”

图

因此，这种对话方法将有助于澄清用户的意图以及构建机器的知识库。对话方法还提供了一种自然机制，以比“历史行动”更容易进行审查和反思的方式来保护用户的迭代过程。

图

通过将界面展开为线性，可遍历的“新闻动态”，用户可以按照自己的想法检查每个阶段，并轻松地返回到较早的迭代，在新方向上分支，同时保留设计的其他版本。


#### 会话界面

在过去的几年中，我一直在努力通过软件来实现这些想法。我创建了一个名为Foil的组合编程语言和设计工具，其目的是将我们讨论的许多概念带入生活，并为用户提供从新手到专家的全方位设计经验。Foil可以根据设计人员的需求量身定做，从而帮助设计人员发展和开发专业知识。根据用户，Foil可以是消费者设计工具，专业设计工具，以及用于创建紧急界面元素和设计小部件的平台，用户将最终能够彼此共享。

我会很快发布一个Alpha版本的Foil。

在纽约大学ITP研究生课程调查中，艺术与技术交叉点的这些想法已经形成了很大的变化。在这个跨学科的背景下，我明白的是，我们正处于巨大的文化融合的开端，一时间不同的媒体的工具和词汇彼此之间更加相关和互操作。为此，Rune Madsen和我已经开始了一个专注于机器学习设计的研究小组。我们目前还在共同教授一个叫Retninking Production Tools的课程，要求学生调查和开发制作工具的新范例。

过去几年机器学习的技术复兴，带来了令人难以置信的新可能。 但实现这些可能性的真正工作才刚刚开始。这项工作需要跨学科的方法，我非常幸运能够在这个激动人心的转型期间与Rune，教师和ITP的学生合作。

#### 结论

很多人似乎担心人造智能会取代我们的工作，使我们毫无用处。我看到确实不同的可能性，一个更乐观的可能性。 事实上，在未来，我们是更强大的，我们都是更加人性化。在未来，我们不是与机器竞争，我们使用它们来扩展我们的边界，就像我们一直做的那样。但是要到达那里，我们需要提醒自己什么工具。工具并不意味着使我们的生活更轻松。它们是为了给我们带来杠杆作用，以便我们更加努力。
...
#### ❇️ 设计师需要了解的机器学习内容 [What designers need to know about machine learning](https://hackernoon.com/what-designers-need-to-know-about-machine-learning-109a12fdd3af)

机器学习已经被广泛用作Netflix的推荐工具，电子邮件和消息应用程序中的垃圾邮件过滤器，像Facebook等网站上的图像识别等功能的一部分，允许产品和服务随着时间的推移“学习”并相应进行调整。 产品和服务设计方式的这种变化现在开始改变我们从一开始就概念化和设计数字产品和服务的方式。 所以，对于我们的设计师来说，现在该开始了解这项技术如何影响我们工作中的设计和用户体验，以及我们如何使用这项技术为用户创造新的更好的产品。
...
为什么这很有用？ 因为，我们不能用布尔逻辑来编程所有的东西。 例如，识别人脸用布尔逻辑来处理的话是一个太复杂的问题。 机器学习有趣的地方在于，它实际上有点像我们了解周围的世界的心理过程。 我们通过观察一段时间以来经历的事物，学会了解世界。
...
那么我们如何使用这些新工具呢？设计师应该考虑到使用机器学习之后，我们的系统能够从大量的信息源中理解复杂的信息。例如，我们现在可以识别的语言，照片中的面部表情和对象，这就意味着我们可以开始思考超越点击和滑动，为用户提供与产品交互的其他方法。例如，你可以上传你想购买的东西的照片，并要求系统找到最近的可以买到的地方，或者告诉你一个特定的事物叫什么。听觉输入类似地允许您通过与系统交谈来进行交互。 通过身体的输入，如面部表情或通过Apple Watch或Fitbit等设备收集的输入现在可以用作交互方法或评估用户在给定时间的状态的方式。

除了这些新的输入外，机器学习可以让我们以比以往更大的规模发现用户行为中的模式，并且还可以使用这些数据来预测用户未来的行为方式。 我们可以使用这些信息来设计更好的产品和服务，并且更深入地了解我们的接口目前正在被使用，我们的客户是谁，以及基于我们预测用户行为的能力来设计新的功能类型。
...
- 监督学习：当我们能够提供机器学习系统的例子以输入和输出，我们使用这些数据“培训”系统，明确地告诉它什么样的相关性，并要求它来证实这些相关性。例如，我们可以利用有监督的学习来预测房价，例如，提供某个城市所有房价的历史数据。然后，一旦系统根据数据了解房子价格和房屋其他属性之间的关系，该系统可以预测未来的房价，我们可以要求它查看一个未售出的房子具有一些特点，如卧室的数量，地点，大小。

- 无监督学习：我们要求机器在一组数据中发现模式，而不告诉它寻找特定的相关性。这可以用来发现数据集或系统中的模式，它们对人类来说太复杂，无法自己计算出来。无监督学习可以学习作曲家作品中的风格模式，然后以这种风格创作新的作品。它也可以用来提高监督学习的质量。

- 半监督：在这里我们可以使用无监督学习的发现能力来改进有监督学习问题的预测。
...
对于设计师来说，有机会成为一名“商务翻译员”，弥补了技术能力与技术在业务和终端用户中的实际应用之间的差距。我们可以考虑使用机器学习来解决的一些类型的问题：

![图](https://github.com/Tangchen329/Machine-Learning-for-Design/blob/master/Session00_LiteratureReviewOfML4D/Pictures/19-what%20designers%20need%20to%20know%20about%20machine%20learning/11.png)

#### 如何将机器学习作为设计过程的一部分来使用 

我们已经介绍了机器学习系统的一些功能，但还有许多其他功能，可以进入这里。 无论您希望采用何种类型的系统或功能，请尝试尽可能详细地探索机器学习问题，以便定义要用机器学习解决的问题的参数。 考虑您可以向系统提供的输入，以及您要接收的输出（例如，输入可以是图像，输出可以是产品的链接或图像的描述）。 在这一点上，如果您了解您希望使用的机器学习技术的基本原理，则可以将学习系统作为您的设计的组成部分。技术细节将在后期完全确定，同时将设计作为整体考虑。

重要的是，尽可能与机器学习程序员或数据科学家进行协作，因为他们对什么是可能实现的的理解或如何实现您的愿景将在整个设计过程中是无价的。 一起工作，提出原型想法并验证假设。 设计人员可以在开发现有算法和工具的新用途方面发挥一定的作用，并且直观地将技术用于创建更好的用户体验或创建一个全新的产品。 提出问题，并找出您的假设中不正确的地方 - 机器学习将帮助你在一定程度了解可以如何将机器学习思想融入您的日常生活中。

#### 接下来是什么？

机器学习正在改变设计中可能的面貌。 我们可以使用机器学习系统来理解人的行为，并根据这些数据进行预测。 我们还可以使用机器学习所实现的工具，如图像，脸部和语音识别，推荐系统等等。 这些工具的影响还有待确定 - 但是设计人员在日常工作中了解和利用技术越多，终端产品越好。 实验将是发现我们现在和将来如何利用机器学习的过程的宝贵的一部分，理解机器学习将使设计人员能够在技术，业务需求和最终用户之间建立桥梁。

设计师还有更多的了解这一点 - 表面只是在这里被划开！ [Machine Learning for Designers](https://www.oreilly.com/learning/machine-learning-for-designers)这本书是了解更多关于机器学习的宝贵资源（并且是撰写本文的宝贵资源），您可以在这里了解更多关于如何使用[Coursera的机器学习算法](https://www.coursera.org/courses/?query=machine%20learning)，并在此阅读[完整的麦肯锡报告](http://www.mckinsey.com/business-functions/mckinsey-analytics/our-insights/the-age-of-analytics-competing-in-a-data-driven-world)。
...
越来越多的人认识到计算设计系统可以并且应该包括学习的能力，并且在设计中关于机器学习的研究越来越多（正如本专题的论文所证明的那样）。

知识获取和机器学习是支持设计系统变革过程的主要工具。知识获取强调将知识从外部世界转移到系统中，而不依赖于系统内部的转换。知识获取的主要目标是通过增加新知识来扩展系统的运作。

学习虽然基于对事件和反馈的洞察，但着重于影响表现的变革。 “表现”的含义包括系统提供的解决方案的质量以及生成该设计（或设计）的过程的效率。
...
在特定设计环境中触发学习的情况对学习技术的选择有影响，这些学习技巧适用于运行时，学习情境发生的频率。

•失败提出了一个挑战的系统，以确定其背景及其发生的原因。故障可能发生在部分或临时的形式（如在子问题的回溯中），或者可能等同于找不到设计解决方案的失败 - 解决方案可能已知或可能不知道存在。

•在设计过程结束时，成功并不总是被认为是理所当然的。对于许多设计问题，尽管拥有大量的资源，但很难知道如何实现一个良好的解决方案。能够识别有助于实现良好绩效的因素与寻找失败原因同样具有挑战性，即信用和责任分配问题都很困难。

•预期值和实际值之间的差异表明学习状况，可能包括失败，也许是成功的情况。设计系统的一个优点是它们允许监视几乎任何设计参数或其他因素的演变。设计过程可能会产生超出被认为是正常范围的参数值。虽然通常太局限于不能立即评估为好还是坏，但参数波动可以被捕获并用于预测以及对设计的评估。

•提高能力的必要性可能是内在的长期目标，或者可能作为对设计系统目标的要求的扩展而发生。例如，明确改变目标需要调整以满足这些新的要求。

D2。支持学习的元素。

设计学习取决于三个支撑元素：可以评估和更新的表示结构，用于生成新实体的支持知识，例如规则，计划或动作，以及用于决定如何修改的反馈代表性结构。
例如，代表性结构可以是设计规划模块中的启发式，用于学习的支持知识可以是一组以前的设计计划，反馈可以包括对这些设计计划的执行情况的评估。学习组件可以根据可用的反馈更新从该组计划中提取的新的计划启发式的规划模块。
用于设计问题解决的表示根据设计任务和将要应用的领域进行选择。系统开发人员在进行表示决策时可以考虑修改的可能性并不总是如此。反馈和支持知识通常可以更容易地定制，以方便学习。
下面给出了设计中使用的反馈和支持知识的例子：

•批评和赞美反映了设计中的效用因素。他们通常代表一个观点，并可以提供相反的反馈要素。估计可能是评估信息的初步来源。评估提供对目标或一组目标的决策或价值观的评估。所有这些元素可以由外部来源，系统用户或设计系统内部的源提供，并入独立的系统组件中。大多数情况下，在运行时使用批评，赞美，估计和评估。

•完成设计任务后的反馈通常来自设计系统外部，并反映了人类对设计系统的解决方案以及可能的成分决策提供的评估。该反馈可以参考当前的设计解决方案，也可以参考一整套过去的设计解决方案。它可能是所有或部分解决方案。反馈可能针对整个设计系统或某些特定推理组件。

•分析失败和相互矛盾的因素（例如目标或决策）都涉及在设计过程中引入“压力”的因素。特别是在这种分析使用用户提供的设计原理的情况下，这种反馈可以将设计系统的适应范围扩展到可以自动实现的范围之外。

•设计决策的顺序支持从设计的角度出发作为决策过程的改进。可以通过检查决策排序过程来生成实用程序或新编译的决策（“知识编译”意义上的）。

•设计历史（例如信息流的痕迹，设计系统之间的知识交流和协商），为洞察设计过程的整个部分提供依据。仅通过查看整个设计会话中记录的信息可以获得表征全球设计性能的某些方面。还可以通过查看反映设计的具体方面（例如，通信，信息检索，协商等）的踪迹的集合来确定设计活动的模式。


D3。什么学到了

学习过程的对象可以是被认为对设计性能至关重要的任何方面，因此选择将始终相对于设计系统的目标。
学习的重点不只在一个目标上。任何对设计质量或过程性能至关重要的元素都可以被包含在单独的学习过程中，或者可以在学习过程中整合和修改元素集合。
下面的例子说明了在学习过程中优先考虑的一些要素：

•与设计相关的参数或其他元素的限制是每个设计问题的核心。一些约束可能不可见或已知，因为它们仅在运行时动态发生。其他约束保持隐藏，因为它们分布在多个设计系统组件上。学习可以使他们明确。

•设计参数之间的相关性对于设计过程的协调阶段至关重要。违反这种依赖关系通常导致不充分的决策，使得追溯和重新设计更有可能发生。因此增加依赖关系的知识是值得的。

•支持或反对一项决定，无论是公用事业还是作为理由，都是指导设计过程的基础。支持的任何变化都可能影响最终的设计结果。

•设计规则，方法和计划是几乎任何设计推理系统的成分之一。更普遍地，任何扩展设计系统的基本问题解决知识可能是学习的主要目标。

•类比关联表示在不同情况下类似的推理和/或行为模式。类比的发现代表抽象推理机制并将其转移到设计系统的其他部分或新设计上下文的起点。

•首选项提供了选择过程中使用的排名。首选项可能只是部分显式，由于值范围很大，只能根据需要进行计算。或者，当它们用于表征设计系统的另一部件的选择过程时，它们可能最初是未知的。在任何一种情况下，在设计过程中变得明确或暗示的偏好可以被学习为减少未来不确定性的手段。这应该减少失败和冲突。

•规则，操作和任务的前提条件和后置条件对于限制推理搜索空间和避免失败至关重要。两种类型的条件可以通过经验来改进或学习。

•设计决策的后果有助于确定设计行动的效用。 了解设计决策的最终结果有助于指导设计开发的评估和预测。

•故障和冲突可以分为类型，然后允许识别可能创建它们的典型情况。

•失败恢复和冲突解决的启发式通常是通过观察以解决方案结束的情况的“记录”来产生的。 失败恢复和解决冲突经常只有有限的前瞻性，事后合成的信息可以证明对未来的情况非常有用[Cross 1978]。

•成功的设计和设计过程可以简单地作为案例学习，或者可以用作知识，以支持在重用时非常有可能导致成功结果的新结构（例如，通过泛化）的生成。


D4。学习知识的可用性
学习机制取决于支持知识的可用方式。这对现有知识的数量，偶尔（定期或永久）的频率，其范围（地方或全球）及其有效性的时间限制（适用于有限的情况）有影响或总是）。在某些情况下，它也可能会影响其质量。

•直接通信代表设计系统和用户之间或设计系统模块之间的消息流。信息的持续性很短，必须存储或立即用于学习。内容可以参考设计或设计过程的几乎任何方面。

•间接通信（例如，通过黑板在设计系统之间）通常传达时间关键时间较短的信息，并且其频率往往低于直接通信。知识数量，其范围或时间的有效性与直接沟通的情况一样不受限制。

•设计状态的记录提供了设计演变以及设计动作的背景和影响的形象。设计状态直接反映了设计的质量，间接反映了设计过程。它涵盖从地方到全球层面的设计方面。这种对学习支持的持续性很低，因此自适应组件必须立即处理可用信息，或必须存储状态的相关方面供以后分析。

•从时间限制的角度来看，设计和交互历史存储库为生成学习信息提供了无限制的机会。存储的信息通常限于在录制时被认为相关的方面。这种知识来源通常是非增量学习技术最有用的设置。

D5。学习方法
实际上任何学习技术都可以应用于设计。到目前为止，并不是全部使用了相同的程度，主要是由于研究人员注意到设计学习领域的时间相对较短。以下列表介绍了许多可能的示例。

•基于说明的学习在设计问题上具有潜力，其中设计状态和行为的逻辑表示被证明是有效的[Mitchell等人1986]。

•感应[Fisher 1987]一直是最广泛使用的技术之一，从开发新设计概念，预测未知设计参数或建模设计代理人的行为（Grecu＆Brown 1996）。

•知识编译[Brown 1991]可用于在规划中生成宏操作符，或将知识重组为设计分解或配置。

•基于案例和类比学习是设计中知识重用的基本技术之一[Maher＆Pu 1997]。

•加强学习支持设计中的行动选择，重点在于设计生成过程，以及正确的反馈可用的系统[Tan 1993; Whitehead 1991]。

•遗传算法通过重新组合完整描述设计的初始集合来实现设计适应[Bentley＆Wakefield 1995]。它们也可能适用于设计生成知识，如语法[Brown 1997]。

•当学习设计配置模式或行为模式可以提高设计性能时，神经网络是一种相关的方法[Ivezic＆Garrett 1994]。

D6。本地与全球学习
设计中的绝大多数学习应用程序描述了在个人设计系统或模块中的学习。分布式设计系统和面向代理的设计系统最近引起了越来越多的兴趣，部分实施学习的努力已经转移到这个新的领域。实际上，在单个设计系统中实现的所有学习范例在映射到单个设计代理时保持其相关性。我们称这种学习本地学习。
多代理设计系统涉及至少涉及两个代理的一组流程。这些过程中的一些意味着几个代理商的积极参与，而其中一些代理人则意味着一个代理人对另一个代理人的知识的使用。在这两种情况下，与此类过程相关的学习都是分布式学习过程。
在分布式学习的“极端”阶段，全球影响是由个人代理人的地方变化而产生的[Hutchins 1991; Shoham＆Tennenholtz 1993; Weiss 1993]。这些影响可以被认为是由地方层面的部分观点和反馈引起的全球学习，但仍然导致了整个系统的新一致行为。

D7。 学习的后果

设计系统的性能和学习机制的成功可以通过两个不同的尺度来衡量：

•设计改进意味着更高质量的设计解决方案。 设计质量措施可以提供关于学习对设计的各个方面的影响的反馈。 尽管单一的全球设计质量指标很少可用，但描述所生产设计的各个参数的变化提供了所学习的多方面视图。

•通过学习改进设计过程意味着提高设计系统效率。 在某些设计系统中，这可以是避免资源限制的一种方式。 为了实现改进的设计质量，重定向这些“保存的”资源越来越重要，因为尝试了更难的设计问题。

4.0结论

本文提出了一套设计研究中机器学习的维度，用于比较现有工作的指导，并为该领域的探索提出了新的方向。 这些维度可能不完整，可能还有其他可能的分析，但我们认为这里提出的这些是有用的贡献。

包括学习在内的设计系统的数量以及在设计系统中学习的许多可能用途表明，这是适时的尝试以系统的方式分析这些发展和机会。


#### ❇️ 设计师的机器学习应用 [Applications Of Machine Learning For Designers](https://www.smashingmagazine.com/2017/04/applications-machine-learning-designers/) By Lassi Liikkanen

本文通过应用检测、预测和生成来说明机器学习的能力。它给出了机器学习使产品和服务更好的六个原因，并介绍了与这类应用相关的四种设计模式。为了帮助您开始，我已经准备了两个非技术性的问题，这些问题将有助于评估您的任务是否准备好由机器学习。

#### 大数据，小智能

大数据和巨大的承诺。 一旦大数据涌入了一大堆数据流，我们预计会发生很多事情。 数据可以以很多方式使用。 一个是建立智能产品，另一个是做出更好的设计和业务决策。 后者也最终影响产品。
机器学习是一种非常有前途的方法，从根本上塑造未来的产品和服务开发。 机器学习是人工智能的一个分支。 它采用了许多方法：深度学习和神经网络是两个众所周知的实例。机器学习意味着，计算机本身不需要为其提供关于如何执行任务的非常详细的说明来学习任务。 最近的一个显着的里程碑是当Google的AlphaGo软件学会了在世界冠军级别掌握围棋游戏！

> 几乎所有正常人都可以在<1秒钟内完成任何事情，现在我们可以使用AI自动执行。
> 吴恩达 Andrew Ng（@AndrewYNg），2016年10月19日

建议您阅读我对该主题的[介绍](https://sc5.io/posts/basics-of-machine-learning/)或Nvidia对[ML，AI和深度学习之间差异](https://sc5.io/posts/basics-of-machine-learning/)的明确解释。

#### 为什么我需要了解机器学习的应用？
作为一名设计师，您将面临更多的需求和机会，与体现机器学习的数字系统合作。随着关于机器的不是宣传加剧，这将导致技术驱动的压力，广泛地利用机器学习。这可能发生在几乎不了解其实际利益及其对产品需求和客户体验的影响的情况下。

#### 留在机器智能游戏的顶端！
作为一名设计师，您有机会了解有关机器智能的计划以及如何在人机接口层上实现最佳实践，您需要知道它能做什么以及数字服务如何利用它。这篇文章以具体案例提供了应用机器学习的概，，以及您可以使用的新设计指南。这将有助于进行实际的设计决策并确定正确的设计模式，包括当没有直接适用的解决方案存在的情况，您必须跨领域传达想法。

#### 解决问题的核心
为了利用机器学习的能力，您必须掌握计算机能理解的任务性质。我用学习的核心问题来描述这个概念。这是指定义我们希望计算机学习什么才能完成我们分配给它的任务。在成品的实际，整体水平（特斯拉的自动驾驶仪功能）上，这些目标并不总是显而易见的。这个故事将帮助您了解表面上发生的事情。
...
#### 机器学习进展？
在2016年，机器学习中最着名的里程碑是AlphaGo赢得了围棋世界冠军Lee Sedol。考虑到围棋是一个非常复杂的游戏，这是一个了不起的成就。除此之外，Google Image Search也许是最着名的机器学习应用。当它有效地隐藏所有的复杂性嵌入，搜索感觉如此自然和平凡，。 Google Image Search每天搜索查询次数超过300亿次，不断获得更多的学习机会。
已经有比这里列出的更多的个人机器学习应用程序。但是，我觉得一个[主简化](https://hbr.org/2016/11/what-artificial-intelligence-can-and-cant-do-right-now)是不够的。寻找分类的一种方法是从Eric Siegel的2013年的书“预测分析”（Predictive Analytics）中看到成功的ML应用程序。列出的应用程序包含以下领域：
- 营销，广告和网络;
- 财务风险和保险;
- 卫生保健;
- 犯罪和欺诈侦查;
...
他的跨行业收集的例子是对预测应用无所不在的强大说明，尽管他的147个例子并不全部利用机器学习. 然而，对于设计师来说，知道您的问题域是否列在九个列表中，将会提示机器学习是否已经被证明是有用的，或者您是否面临着一个很大的未知数。
...
#### 检测分类
检测应用至少有四种主要应用。每种处理不同的核心学习问题。他们是：
- 文字和言语解释，
- 形象和声音解读，
- 人类行为和身份检测，
- 滥用和欺诈检测。
...
文字和言语是最自然的交互和沟通方式。 因此，在电脑领域是不可行的。 前几代的语音拨号和交互式语音应答系统并不令人印象深刻。 只有在这十年中，我们才看到了新一代的应用程序，使用口头命令甚至与我们进行对话！ 这可以顺利进行，我们不能在基于文本的聊天中分辨计算机和人员，表明计算机已经通过了图灵测试。

处理语音，新系统如个人助理Siri或亚马逊的回声设备能够解读广泛的通信和智能响应。 该功能的技术术语是自然语言处理（NLP）。 这表明，基于成功的文本和语音检测（即识别），计算机还可以解释口头和书面的单词的含义，并采取行动。
...
鉴于计算机正在看到和听到我们所做的工作，所以他们已经能够分析和检测人类的行为和身份 - 例如，Microsoft Kinect可以识别我们的身体动作。机器可以识别足球游戏中的动作，以自动生成游戏统计信息。 Apple的[iPad Pro](https://backchannel.com/an-exclusive-look-at-how-ai-and-machine-learning-work-at-apple-8dbfb131932b)识别用户是否使用手指或铅笔进行控制，以防止不必要的手势。大量的服务可以检测出通常在购物车中携带什么样的物品？这使得亚马逊建议您可能感兴趣的同类产品。
...
文本和语音预测为智能设备的交互开辟了新的机会。 会话界面是这一发展的最突出的例子，但绝对不是唯一的。 当我们尝试隐藏用户的界面和底层复杂性时，我们将平衡我们隐藏的内容和我们所揭示的内容。 建议的功能可帮助用户发现隐形UI的功能。


图形用户界面（GUI）使得有正常视力的人类可以更好的计算机。GUI在功能发现方面提供了巨大的可用性改进。 图标和菜单是使用鼠标和键盘直接操纵数字对象的第一个有用的象征。 通过多点触控屏幕，我们已经获得了挤压，拖动和滑动的新功能。 视觉线索不会消失，但是当互动模式扩大时，它们不够。

##### 用户如何弄清楚您的服务可以做什么？

面向第一代消费者可穿戴产品中的触觉交互和最重要的会话界面为特征发现提出了新的挑战。 必须使用促进交互的非视觉提示，特别是在交互式关系的开始时。 功能建议 - 机器暴露其功能并通知用户它是什么 - 是这个谜语的一个解决方案。在租用汽车的Chatbot的情况下，这可能是“请问有关车辆，升级和您以前的预约”。具体应用领域具有特定的详细模式。 例如，[帕特里克·希布伦（Patrick Hebron）最近的O'Reilly电子书](http://www.oreilly.com/design/free/machine-learning-for-designers.csp)包含了对话界面解决方案的很好讨论。


预测不同形式的人类行为大致归结为以下核心学习挑战和应用：

- 建议
- 个人行为和条件
- 集体行为预测。

不同类型的建议是关于预测用户偏好。当Netflix推荐电影或Spotify生成您未来喜欢的音乐的播放列表时，他们正在尝试预测您是否喜欢，观看或聆听该片段的结尾。 Netflix随后着手对电影的评级，而Spotify或Pandora可能会衡量您是否一次又一次地重复欣赏同一首歌曲，而不会跳过。这样，我们的行为和偏好即使没有我们明确地表达也会变得相关。这是机器可以学习和利用的东西。


在设计中，预测哪些内容或哪些互动模式吸引用户可能会引起接口的个性化。这主要是基于预测用户最感兴趣的内容。几年前，亚马逊已经大量个性化了首页，预测在预期您的购物欲望和习惯中应该存在什么内容和链接。
...
预测集体行为成为个体的泛化，但具有不同的含义和范围。在这些情况下，如果想影响大多数人群，干预措施才是成功的。总统大选，蜂窝网络使用或季节性购物支出的即将到来的结果都可以预测。当预测财务风险或公司的关键业绩指标时，储蓄或赚钱的收益是显着的。摩根大通（JPP.Morgan Chase）是首家通过预测抵押贷款违约者（从未偿还的人）和再融资者（那些偿还太早的人）提高效率的银行之一。另一方面，最近美国的总统大选是一个很好的提醒，没有一个是完美的。


与防病毒和其他现有危险的工具非常相似，未来的麻烦也是可以预见的。预测性警务是关于预测街头冲突可能发生的地方，还是寮屋居民接管的地方，这将有助于管理人员将资源分配到正确的地方。能源公司正在进行类似的过程，因为他们试图估计持续一夜的能力。


#### 设计模式：个性化

一旦电脑了解您并能预测您的愿望和偏好，就可以开始以更有效的方式为您服务。 这是个性化，一种服务的自动优化。 响应式的网站布局是这种类型的较为粗糙的方式。
...
假设您现在确信您的用户和客户将受益于ML带来的升级服务。接下来，您必须考虑业务和技术的前景。要问的第一个问题是，如果ML和您想要的一样，您的产品会增加什么价值？说实话：传统的商业逻辑也适用于这里。

如果机器学习没有创造价值，那么这可能是浪费资源的。营销人员可能会喜欢您的新实施，但业务人员不一定资助您的下一次机器学习实验，除非您有一个商业案例，为什么机器学习将直接改善客户体验或收入。

好的，假设你已经清除了可行性复选框。我期望你至少也有一个核心学习挑战的假设。这可能是，例如，计算机可以学习如何预测用户何时在早上离开房子时需要拿伞或防水夹克。

接下来，您将需要弄清楚是否可以期望机器学习您希望完成的工作。我们来了解数据问题。数据广泛地指您可以为计算机提供的任何信息：天气数据，雨伞和防水装备的购物数据，社交媒体更新等。
...
第一个问题看起来很容易，但很难提前回答。您需要质量好，充足的数据量。有些信号是嘈杂的（也就是质量差）。在这些情况下，您可能需要比其他情况更多的例子。一些数据特征非常突出，易于学习，例如在夜间或日间检测照片时。这可以用少至30个训练图像来解决！
...
如果您对以上问题中的至少一个有正面的回答，那么您可以有一些信心，机器学习可能确实对您有用。有了这个发现之后你会做什么大都超出了这篇文章的范围。

为了构建机器学习解决方案，最好是大胆地去做原型实现所需的功能。如果您有足够的样本，您的数据科学家队友将很快发现机器是否显示熟练的学习任务（特别是如果您使用可扩展的计算机）。如果没有，您需要获得更多的数据。

以下是对学习机制的一系列迭代以及将其与产品或服务集成的过程。智能应用的外观可以通过仔细地整理几台机器学习甚至常规编程来实现。这被称为综合方法。对于用户来说，最好的一个无缝互动体验可能实际上是一个非常复杂的不同机器学习组件在一起工作的产物。

人力智能需要调整学习算法。 这就是沃森的架构，由IBM创造的Jeopardy赢得的人造智能系统以及声称Netflix奖的算法。 在后一场比赛中，最后几个月内，有几支队伍结合了他们的个人调整，最终超过了标准，获得了成功。 这也是Google翻译团队提升到一个新的水平的原因。 而这项工作是我认为属于传统软件开发领域。
...
从事服务和界面设计的设计人员将会在现在的头脑中提出几个关于互动的问题。我们如与用户交流机器学习，机器学习将需要什么样的新界面？
...
Photoshop是一个具有陡峭学习曲线和大量细粒度控制功能的优秀示例。大多数时候，用户都在做小型的操作，每个操作都有非常具体的影响。许多小事物的创造性组合允许有趣的模式更大规模地出现。整体的黑匣子操作，如变换过滤器和自动更正并不是专业人士使用Photoshop的原因。
...
我称最终模式叫优雅的失败。这意味着用一种让人可以理解的方式说“对不起，我不能做你想要的，因为...”

这并不是机器学习应用程序的独特之处。从20世纪80年代，Commodore电脑的语法错误反复出现以来就臭名昭着。但是通过机器学习，它略有不同。因为机器学习采用模糊逻辑的计算方法，计算机可能会产生意想不到的结果 - 也就是说，事情可能会变得非常糟糕，而且必须设计出来。没有人因为2016年特斯拉自动驾驶仪事故中发生的死亡而责怪该车。

另一方面，依靠现代机器学习的建筑应用仍处于起步阶段。经典软件开发已经存在了很长时间，我们已经学会了更好地应对其不足之处。着名的AI研究员和Google研究总监Peter Norvig表示：

> 这里的问题在于扩展到整个行业的解决方法也仍在改进中...我们在开发和验证常规软件方面都还没有数十年的经验。

学习的本质就是电脑学习给予他们的东西。如果算法必须处理其他的东西，那么结果将不符合你的想法。例如，如果您已经培训了一个系统来检测宠物照片中的动物物种，然后开始使用它来对植物进行分类，就会有麻烦。这或多或少是为什么微软的Twitterbot Tay在暴露于现实世界的条件之后，从恶意用户那里拿到错误的例子后，不得不销声匿迹。

应考虑检测和预测的不确定性。怎么做取决于应用程序。考虑Google搜索，没有人被冒犯或真正的伤害，只会因为不好的搜索结果感到被戏弄或沮丧。当然，糟糕的结果最终会对企业造成不利影响。但是，如果您的银行开始使用一个突然无法找出支票账户余额的聊天人，那么您将会非常担心，并应尽快解决您的问题。


为了应对失败，接口将很好地帮助双方进行调整。用户可以容忍一个或两个“我没有听清，请再说一次”提示（但不再有），如果这是推进对话所需要的。对于包括机器学习在内的服务，广泛的测试是最好的。接下来会通知用户故障的概率和后果，以及用户可能会采取什么措施来避免失败。良好做法仍在不断涌现。

#### 总结
通过机器学习，我们的明天愿景正在迅速成为今天的现实。

总的来说，几乎似乎没有机器学习不能用于检测或预测的应用程序。 我引入了很多示例来部署机器学习来取得不同的成功。 在所有的例子中，我试图说明一些核心学习的挑战，以帮助您了解机器响应的任务。 现在是时候面对什么机器学习可以为你做的事情！我不认为这个指南本身就足以在智能产品和服务领域进行激进的创新。 我希望这将打开几位设计师的眼睛，看看机器学习解决方案今天能为我们带来的机会。 了解大概您可以从机器实际询问的内容很重要。 在不久的将来，好的问题将变得越来越重要，所以答案将是：“是的，戴夫。 我可以做到这一点。
...
- 设计机器学习系统的用户体验是AAAI会议在斯坦福大学举行，斯坦福，加利福尼亚从3月27日–29, 2017。

讨论主题：探索，实验和在系统间的合作的仿真和解决冲突需要更好的工具的需求；更好的模式和围绕着数据清洗，维护，管理，和源审计的实践；并建议如何加深合作。
...
- 偏差、信任与权力
我们还解决了更多的哲学和政治问题。在研讨会的第二天，我们讨论了系统透明性，呼吁明确出处模型，明确在机器学习数据集，源和交互的潜在偏差。基本的要求是始终提供多个观点，以减轻与偏见有关的问题，并使偏见成为调查本身的明确主题。
信任和权力的关键问题的研讨会：什么关闭对话框应该系统与他们的用户，并且什么它意味系统要“风度”，有“个性”，而“社会责任”？
...
#### ❇️ 机器学习如何加速设计周期 [HOW MACHINE LEARNING CAN SPEED UP YOUR DESIGN CYCLE](https://www.maximintegrated.com/en/design/blog/machine-learning-can-speed-up-design-cycle.html) By: Christine Young

Elyse Rosenbaum, professor of electrical and computer engineering at the University of Illinois at Urbana-Champaign说机器学习将促进更快，更准确的基于模型的设计，更精确的基于仿真的设计验证，和更有效率的计算系统级制造变化分析。

机器学习：对电子建模和设计优化有利的一项技术。她说，在适当的情况下，机器学习可以提高电子设计自动化（EDA）。                                             现代EDA技术，Rosenbaum说，有其局限性，即：
-它还没有消除设计周期
-在资格测试中观察到的许多失败直接源于建模能力不足（而且可变性不能精确地建模或以计算效率的方式进行）。
-基于仿真的设计优化表现出有限的成功；通常，由于涉及到大量的设计变量，它是缓慢而不切实际的。
-行为模型解决了当前模拟能力的一些缺陷。然而，该行业缺乏产生这些模型的一般的、系统的方法。
根据罗森鲍姆的说法，这是因为这些挑战：
-输入空间的高维和输入参数之间相关性的有限知识
-系统组件和子系统的物理属性的可变性
-高非线性响应曲面的采样和表示困难
-缺少关于电磁或其他组件间相互作用的先验信息

- 机器学习的拯救

这就是机器学习可以发挥作用的地方。根据罗森鲍姆的说法，“机器学习”出现在1959在IBM的研究与开发杂志第一次使用：“使用的跳棋游戏，学习机研究”。多年来，流行文化已共享的解释（记得星际迷航记在队长Kirk告诉坐在后面让机器做的工作吗？）。在罗森鲍姆看来，机器学习是统计学习理论的构建准确的预测中的应用。有了足够的训练数据，即使输入和输出之间存在非常复杂的函数关系和/或存在随机效应，这种方法也不会受到影响。

比起现在用机器学习的很多应用场合，EDA是更佳适合的领域”罗森鲍姆说。使用机器学习来提取支持EDA的模型可以使工程师的专业生活更好，使设计优化和更短的市场上市时间。                                                    
有多种机器学习算法和分析，可以应用在EDA流中。线性回归，logistic回归，神经网络，核方法，监督和无监督学习是一些例子。                                              考虑机器学习如何支持容易产生自加热的三维集成电路的热设计优化。罗森鲍姆解释说，她在佐治亚的同事进行电力输送网络的第一个3D热模拟，利用产生的热曲线进行电路仿真测量时钟偏移。他们必须反复经历模拟过程，直到他们确定了一个符合时钟偏移要求的设计。使用一种叫做贝叶斯优化的统计学习方法，研究人员减少了模拟的数量。唯一的规定，罗森鲍姆说，是你要的功能模型准确误差接近最小值。

另一个例子是罗森鲍姆涉及到了代理模型的电路设计，其中数据确定模型的结构。在射频电路的设计，包括调谐旋钮是惯例。然而，使用SPICE模拟，很难确定最佳的调谐旋钮集，因为所需的模拟数量将太大。罗森鲍姆的同事在北卡罗莱纳州建立了一个替代的RFIC器件模型，通过这种方法，确定了三项调整旋钮，将满足其性能要求的设计。

递归神经网络在EDA中也很有用，因为它可以近似使用状态空间模型表示的任何系统，它涵盖了许多电路和设备。在RNN模型商业缓冲芯片为例，RNN模型模拟和12倍更快的晶体管级模型（HSPICE仿真），罗森鲍姆指出。

- 物理与经验模型

罗森鲍姆的一个主要优点是，即使有从机器学习芯片设计明显的好处，也有它的时间和地点。“我们承认，物理模型是有价值的，”罗森鲍姆说。“记住经验模型只不过是真实输入/输出函数的近似值。如果你能很容易地从你自己身上获得物理模型，你就不应该使用经验模型。”
显然，机器学习给芯片设计过程以及集成电路本身带来了好处。马克西姆当然知道它的许多技术应用机器学习技术的价值。该公司甚至还为员工举办了一场机器学习竞赛。
...
随着人工智能时代的到来，设计与人工智能的纠葛，艺术与科技的博弈，越来越频繁地现身热门话题榜。而在设计人工智能领域，阿里巴巴已经开始加速探索。15 年“双11”，是阿里第一次基于算法和大数据，为用户做大规模的、个性化的商品推荐，叫做“千人千面”，是阿里流量分发模式很大的升级和转型。双11”结束后，我们（设计、工程和算法团队）就聚在一起，商量下一年要做些什么。我们当时想，我们已经做到的个性化推荐，但都是基于白底图商品推荐，能不能往前迈一步，让强营销导向的广告资源位的设计也“千人千面”呢？从纯商品个性化跨到广告资源位个性化，中间几个关键的技术点打通之后，我们就着手做了。
...
![36](https://github.com/Tangchen329/Machine-Learning-for-Design/blob/master/Session00_LiteratureReviewOfML4D/Pictures/36-luban/2.jpg)
...
![36](https://github.com/Tangchen329/Machine-Learning-for-Design/blob/master/Session00_LiteratureReviewOfML4D/Pictures/36-luban/3.jpg)
...
![36](https://github.com/Tangchen329/Machine-Learning-for-Design/blob/master/Session00_LiteratureReviewOfML4D/Pictures/36-luban/4.jpg)
